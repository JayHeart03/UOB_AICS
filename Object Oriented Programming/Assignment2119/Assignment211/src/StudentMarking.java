...
public class    StudentMarking {
   ...
   */
    ...
    public final String STUDENT_ID_TEMPLATE = "You entered a given name of %s and a surname of %s%n" +
            "Your studID is %s";
    public final String NAME_RESPONSE_TEMPLATE = "Please enter your given name and surname (Enter 0 to return to main menu)%n";
    ...
    public String generateStudId(Scanner sc) {
        ...
        char Give_Name_Initial;
        char Surname_Initial;
        String Family_name_length;
        char Given_Name_Middle_Letter;
        char Surname_Middle_Letter;
        System.out.printf(NAME_RESPONSE_TEMPLATE);
        String GivenName = sc.next();
        if (GivenName.equals("0")) {
            return studId;
        }
        Give_Name_Initial = GivenName.toUpperCase().charAt(0);
        Given_Name_Middle_Letter = GivenName.toLowerCase().charAt(GivenName.length() / 2);

        String Surname = sc.next();

        Surname_Initial = Surname.toUpperCase().charAt(0);
        Surname_Middle_Letter = Surname.toLowerCase().charAt(Surname.length() / 2);
        Family_name_length = String.format("%02d", Surname.length());
        studId = "" + Give_Name_Initial + Surname_Initial + Family_name_length + Given_Name_Middle_Letter + Surname_Middle_Letter;
        System.out.printf(STUDENT_ID_TEMPLATE, GivenName, Surname, studId);
        return studId;
    }
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            sm.displayMenu();
            Received_Option = sc.nextInt();
            if (Received_Option == EXIT_CODE) {
                break;
                ...
                if (!studId.equals("generateStudId is incomplete")) {
                    keepStudId[keepStudIdNumber++] = studId;
                    count++;
                }
            } else if (Received_Option == 2) {
                ...
                for (int i = 0; i < count; i++) {
                    if (keepStudId[i].equals(studId)) {
                        keepAvg = sm.captureMarks(sc, studId);
                        flag = 1;
                    }
                }
                if (flag == 0)
